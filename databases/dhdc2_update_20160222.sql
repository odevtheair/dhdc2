...
-- ----------------------------
INSERT INTO `sys_version` VALUES ('20160222');
...
,CONCAT(
	...
	,if(t.GRAVIDA NOT IN (1,2,3,4,5),'AN1143,','')
	,if(p.HOSPCODE IS NUll or p.PID is NULL,'AN9200,','')
	,if(hos.hoscode is NUll,'AN9201,','')	
) as ERR_CODE 
	,t.HOSPCODE 	,t.PID ,t.DATE_SERV,t.GA,t.GRAVIDA,date(t.D_UPDATE) as 'DUPDATE'
FROM  anc  t 
...
,CONCAT(
	...
	,if(t.DATE_SERV is NULL or t.DATE_SERV < (CURDATE()-INTERVAL 50 YEAR) or  t.DATE_SERV > CURDATE(),'CF1105,','')
...
/*
INSERT INTO err_dental (
	SELECT 'DENTAL' as FILE ,('DT1105,') as ERR_CODE
	,t.HOSPCODE,t.PID,t.SEQ,s.SEQ as SERVICE_SEQ,t.DATE_SERV,t.DENTTYPE,t.SERVPLACE
	FROM dental t 
	LEFT JOIN service s on t.HOSPCODE = s.HOSPCODE AND t.PID = s.PID AND t.SEQ = s.SEQ
	WHERE t.DATE_SERV is NULL  or t.DATE_SERV < (CURDATE()-INTERVAL 10 YEAR) or  t.DATE_SERV > CURDATE()
);*/
...
,CONCAT(
	...
	,if(t.DATE_DETECT is NULL or t.DATE_DETECT < (CURDATE()-INTERVAL 50 YEAR) or  t.DATE_DETECT > CURDATE(),'DS1105,','')
...
,CONCAT(
	...
	,if(t.LMP is NULL or t.LMP < (CURDATE()-INTERVAL 50 YEAR) or  t.LMP > CURDATE(),'LB1105,','')
	,if(t.BDATE is NULL or t.BDATE < (CURDATE()-INTERVAL 50 YEAR) or  t.BDATE > CURDATE(),'LB1106,','')
...
,CONCAT(
	...
	,if(t.GRAVIDA is NULL OR (CONCAT('',t.GRAVIDA * 1)<> t.GRAVIDA),'PO1104,','')	
	,if(t.BDATE is NULL or t.BDATE < (CURDATE()-INTERVAL 50 YEAR) or  t.BDATE > CURDATE(),'PO1105,','')
	,if(t.PPCARE is NULL or t.PPCARE = '0000-00-00' or  t.PPCARE > CURDATE(),'PO1106,','')
...
-- ----------------------------
-- Function structure for getwordnum
-- ----------------------------
DROP FUNCTION IF EXISTS `getwordnum`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `getwordnum`(param1 text,param2 double) RETURNS char(200) CHARSET utf8
BEGIN
	declare cvalue text;
	declare cword text; 
	set cvalue=substring_index(param1,',',param2);
	set cword=if(param2<2,'',substring_index(param1,',',param2-1));
	set cvalue=substring(cvalue,length(cword)+1);
	set cvalue=replace(cvalue,',','');
	set cvalue=replace(cvalue,'.000','');
	return cvalue;
...
-- ----------------------------
-- Function structure for hosxpdatacenter
-- ----------------------------
DROP FUNCTION IF EXISTS `hosxpdatacenter`;
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `hosxpdatacenter`(param1 text,param2 double) RETURNS char(200) CHARSET utf8
BEGIN
	declare cvalue text;
	declare cword text; 
	set cvalue=substring_index(param1,',',param2);
...
DELIMITER ;;
CREATE DEFINER=`root`@`localhost` FUNCTION `nutri_cal`(agem int(3),isex varchar(1),ntype int(1),h int(3),w int(3)) RETURNS varchar(1) CHARSET utf8
    DETERMINISTIC
BEGIN
	     DECLARE chkrs int(1) DEFAULT 0;
